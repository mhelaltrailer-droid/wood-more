/// مرفق واحد ضمن سجل MoS-ITP.
class MosItpAttachmentModel {
  final int id;
  final int recordId;
...
/// سجل MoS أو ITP مع مرفقاته.
class MosItpRecordModel {
  final int id;
  final int projectId;
  final int? userId;
  ...
  final String? notes;
  final DateTime? createdAt;
  final List<MosItpAttachmentModel> attachments;

  const MosItpRecordModel({
    required this.id,
  ...
  static const String kindMos = 'mos';
  static const String kindItp = 'itp';
  ...
  factory MosItpRecordModel.fromMap(Map<String, dynamic> m) {
    int pInt(dynamic v) =>
        v is int ? v : int.tryParse(v?.toString() ?? '') ?? 0;
    ...
    final atts = <MosItpAttachmentModel>[];
    if (rawAtt is List) {
      for (final e in rawAtt) {
        if (e is Map) {
          atts.add(
            MosItpAttachmentModel.fromMap(Map<String, dynamic>.from(e)),
          );
        }
      }
    }
    return MosItpRecordModel(
      id: pInt(m['id']),
