...
/// MoS-ITP: Document Controller يرفع، الباقي يعرض فقط، المسؤول المحدد يعدّل/يحذف.
class MosItpScreen extends StatefulWidget {
  final UserModel currentUser;

  const MosItpScreen({super.key, required this.currentUser});

  @override
  State<MosItpScreen> createState() => _MosItpScreenState();
}
...
class _MosItpScreenState extends State<MosItpScreen> {
  final _db = getStorage();
  List<ProjectModel> _projects = [];
  ProjectModel? _project;
  String? _kind;
  List<MosItpRecordModel> _records = [];
  bool _loading = false;
  String? _loadError;

  bool get _canUpload => widget.currentUser.canUploadMosItp;
  bool get _showAudit => widget.currentUser.canManageMosItpRecords;
  ...
  Future<void> _loadRecords() async {
    ...
    try {
      final list = await _db.listMosItpRecords(
        projectId: _project!.id,
        kind: _kind!,
        requesterEmail: widget.currentUser.email,
      ) as List<MosItpRecordModel>;
      if (!mounted) return;
      setState(() {
        _records = list;
        _loading = false;
            ...
            return Padding(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    Text(
                      'إضافة ${MosItpRecordModel.kindLabel(_kind!)} جديد',
  ...
  Future<void> _confirmDeleteRecord(MosItpRecordModel record) async {
  ...
  Future<void> _showEditRecordForm(MosItpRecordModel record) async {
  ...
  Widget _buildRecordCard(MosItpRecordModel record) {
    return Card(
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Icon(
                  record.kind == MosItpRecordModel.kindItp
  ...
  Widget build(BuildContext context) {
    final kindLabel =
        _kind != null ? MosItpRecordModel.kindLabel(_kind!) : null;

    return Scaffold(
      appBar: AppBar(
        title: const Text('MoS-ITP'),
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  Text(
                    _canUpload
                        ? 'منهجية التنفيذ وخطة الفحص والاختبار — إضافة سجلات جديدة'
                      ...
                      children: [
                        Expanded(
                          child: ChoiceChip(
                            ...
                            selected: _kind == MosItpRecordModel.kindMos,
                            onSelected: (_) {
                              setState(() => _kind = MosItpRecordModel.kindMos);
                              _loadRecords();
                            },
                          ),
                        ),
                        ...
                        Expanded(
                          child: ChoiceChip(
                            ...
                            selected: _kind == MosItpRecordModel.kindItp,
                            onSelected: (_) {
                              setState(() => _kind = MosItpRecordModel.kindItp);
                              _loadRecords();
                            },
                          ),
                        ),
